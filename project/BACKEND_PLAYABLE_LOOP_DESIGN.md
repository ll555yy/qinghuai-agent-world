...
- 现实有效 1 秒对应虚拟 1 分钟，因此 600 秒对应一个 08:00～18:00 世界日。
- 测试和无前端 API 使用 `POST /api/runs/{runId}/world/step` 提交 `realSeconds`；后端换算后推进，不要求测试真实等待。
...
- 胜者发言会成为新消息并继续流水线。为防模型自动无限聊天，每次外部触发最多连续生成 8 条 NPC 消息；达到上限进入等待，不伪造结束台词。
...
1. 600 realSeconds 对应完整世界日，跨越时按序触发事件和五名 NPC 一日一次错峰思考。
