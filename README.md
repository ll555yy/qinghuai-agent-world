...
| `ARK_EMBEDDING_MODEL` | 2048 维私有记忆与 NPC 示例对白 Embedding | 向量检索时必需 |
...
| `NPC_SPEECH_EXAMPLES.yaml` | 每名 NPC 八条人工审核的表达示范 |

NPC 最终决定 `action=speak` 后，后端仅使用最终 `intent` 在该 NPC 的示例中检索
Top 3，并把示例作为风格参考注入台词生成。示例不参与事实、关系、Goal 或记忆判断；
Embedding 不可用时会自动降级为原有的无示例生成流程。
