"""Strict loader and cross-reference validator for the scenario YAMLs."""
...
class ScenarioLoader:
    ...
    def _load_speech_examples(
        ...
        for index, raw in enumerate(rows):
            ...
            example_id = _str(
                row.get("exampleId"), file=filename, field=f"{field}.exampleId"
            )
            if example_id in parsed:
                raise ScenarioValidationError(
                    "duplicate example ID",
                    file=filename,
                    field=f"{field}.exampleId",
                )
            npc_id = _str(
                row.get("npcId"), file=filename, field=f"{field}.npcId"
            )
            actor = actors.get(npc_id)
            if actor is None or actor.kind != "npc":
                raise ScenarioValidationError(
                    "must reference an existing NPC",
                    file=filename,
                    field=f"{field}.npcId",
                )
            parsed[example_id] = SpeechExampleDefinition(
                ...
                npc_id=npc_id,
                situation=_str(
                    row.get("situation"), file=filename, field=f"{field}.situation"
                ),
                intended_move=_str(
                    row.get("intendedMove"),
                    file=filename,
                    field=f"{field}.intendedMove",
                ),
