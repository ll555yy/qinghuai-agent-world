...
from ..persistence.speech_example_retriever import (
...
class RunService:
    ...
    async def _execute_message_round(self, run: Run, conversation_id: str) -> None:
        ...
        try:
            ...
            example_results = await asyncio.gather(
                *(
                    self._retrieve_speech_examples(
                        run_id=run.run_id,
                        conversation_id=conversation_id,
                        round_id=round_id,
                        npc_id=npc_id,
            ...
            async with run.lock:
                ...
                prompts = [
                    ...
                    for (npc_id, decision), result in zip(
    ...
    async def _retrieve_speech_examples(
        ...
        *,
        ...
        try:
            result = await self.speech_example_retriever.search(
                npc_id=npc_id,
                intent=intent,
                limit=3,
            )
        except Exception:
            result = SpeechExampleSearchResult(failure_code="retriever_error")
        logger.info(
            "speech example retrieval",
            extra={
                "speech_example_retrieval": {
                    "runId": run_id,
                    "conversationId": conversation_id,
                    "roundId": round_id,
                    "npcId": npc_id,
                    ...
                    "exampleIds": [
                        hit.example.example_id for hit in result.hits
                    ],
                    "similarities": [
                        round(hit.similarity, 4) for hit in result.hits
                    ],
                    "durationMs": round(
    ...
    async def _execute_opener_round(self, run: Run, conversation_id: str) -> None:
        ...
        try:
            ...
            async with run.lock:
                ...
                self._apply_chat_drafts(run, conversation, npc_id, decision)
                opener_intent = decision.intent or str(state.get("openerIntent") or "")
                state["status"] = "generating"
                await self.repository.save(run)

            example_result = await self._retrieve_speech_examples(
                run_id=run.run_id,
                conversation_id=conversation_id,
                round_id=round_id,
                npc_id=npc_id,
            ...
            async with run.lock:
                ...
                prompt = self._npc_prompt(
                    ...
                    "opening_speech" if opener_kind == "conversation_opener" else "join_speech",
                    {
                        ...
                        "speechExamples": self._speech_example_payload(example_result),
                        **self._chat_context(run, conversation, npc_id),
                    },
                )
