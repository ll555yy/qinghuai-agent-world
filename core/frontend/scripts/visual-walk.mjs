/**
 * Dynamic occlusion QA: the player walks from the front hall (440,454) to
 * the study (480,190) while we capture frames every 400ms. The route crosses
 * the central doorway and passes furniture, exposing any y-sort clipping.
 *
 * Usage: node scripts/visual-walk.mjs
 */
...
const outDir = path.join(repoRoot, 'project', 'visual-qa', 'walk')
...
function makeSnapshot(playerPosition) {
  return {
    runId: 'run_visual_walk',
    stateVersion: 2,
    eventSeq: 2,
    ...
    actors,
    actorStates: {
      player_001: { status: 'present', position: playerPosition },
...
let liveSnapshot = makeSnapshot({ x: 1, y: 2 })
const wsClients = []
function pushSnapshot(playerPosition) {
  liveSnapshot = makeSnapshot(playerPosition)
  const payload = JSON.stringify(liveSnapshot)
  for (const ws of wsClients) ws.send(payload)
}

const agendas = [
  { agendaId: 'agenda_001_literary_society', ownerNpcId: 'npc_001', title: '青槐文社', publicSummary: '以公益书法课和老街故事会为核心。' },
]
...
async function mockBackend(page) {
  await page.route(/\/api\/(?:health|scenario|runs)(?:\/|$)/, async (route) => {
    ...
    if (url.pathname === '/api/scenario/agendas') return fulfill({ chapter: { chapterId: 'chapter_01', name: '七日方案期', startDay: 1, endDay: 7, endsAt: 'Day7 18:00' }, agendas, actors: actors.filter((actor) => actor.kind === 'npc') })
    if (url.pathname === '/api/runs' && request.method() === 'POST') return fulfill(liveSnapshot, 201)
    if (url.pathname === '/api/runs/run_visual_walk' && request.method() === 'GET') return fulfill(liveSnapshot)
    if (url.pathname.endsWith('/world/step')) return fulfill({ run: liveSnapshot })
    return fulfill({ error: { code: 'not_mocked', message: url.pathname, details: {} } }, 404)
  })
  await page.routeWebSocket(/\/ws\/runs\//, (ws) => {
    wsClients.push(ws)
    ws.send(JSON.stringify(liveSnapshot))
  })
}
...
try {
  ...
  await page.screenshot({ path: path.join(outDir, 'walk-00-start-front.png') })

  // Walk front -> study. The route crosses the doorway and passes the
  // central table, the divider lintel and the study desks.
  pushSnapshot({ x: 2, y: 0 })
  for (let frame = 1; frame <= 14; frame += 1) {
    await page.waitForTimeout(400)
    await page.screenshot({ path: path.join(outDir, `walk-${String(frame).padStart(2, '0')}.png`) })
  }

  // Walk back to the front hall (crossing the doorway southbound).
  pushSnapshot({ x: 1, y: 2 })
  for (let frame = 15; frame <= 24; frame += 1) {
    await page.waitForTimeout(400)
    await page.screenshot({ path: path.join(outDir, `walk-${String(frame).padStart(2, '0')}.png`) })
  }

  await browser.close()
  console.log('Walk screenshots written to', outDir)
} catch (error) {
